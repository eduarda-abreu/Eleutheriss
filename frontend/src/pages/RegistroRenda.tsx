...
import {
  ...
  LogOut, ChevronLeft, DollarSign, FileText, Calendar, CheckCircle,
  AlertCircle, RefreshCw, TrendingDown, Lightbulb, Clock, ArrowRight,
} from "lucide-react";
...
const errorStyle: React.CSSProperties = {
  fontSize: "12px", color: "#8B2246", marginTop: "4px",
};
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#F5F0E4", fontFamily: "'Inter', sans-serif" }}>
      ...
      <aside style={{ width: sidebarW, minHeight: "100vh", background: "#1A1A1A", display: "flex", flexDirection: "column", padding: "20px 0", transition: "width 0.3s ease", flexShrink: 0, position: "sticky", top: 0, height: "100vh", overflow: "hidden" }}>
            ...
            >
              ...
              {!collapsed && (
                <>
                  ...
                  {soon && <span style={{ marginLeft: "auto", fontSize: 9, fontWeight: 600, background: "#2a2a2a", color: "#9a8f7e", borderRadius: 4, padding: "2px 5px", letterSpacing: "0.04em" }}>Em breve</span>}
                </>
              )}
            </div>
          ))}
        ...
        <div style={{ padding: "0 8px", display: "flex", flexDirection: "column", gap: 4 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 10, cursor: "pointer", overflow: "hidden", whiteSpace: "nowrap" }} onClick={() => setCollapsed((c) => !c)}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 10, cursor: "pointer", overflow: "hidden", whiteSpace: "nowrap" }}
      ...
      <main style={{ flex: 1, padding: "32px 32px 48px", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
          <div>
            <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 26, fontWeight: 700, color: "#1a1a1a", margin: 0 }}>Registrar Renda 💰</h1>
            <p style={{ fontSize: 13, color: "#C89B30", fontWeight: 500, marginTop: 4 }}>Adicione uma nova entrada de renda manualmente</p>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 24, alignItems: "start" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 18, boxShadow: "0 2px 24px rgba(0,0,0,0.07)", padding: "36px 40px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, background: "linear-gradient(135deg, #C89B30 0%, #E8BE45 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <DollarSign size={22} color="#1a1a1a" />
              </div>
              <div>
                <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: "#1a1a1a", margin: 0 }}>Nova Renda</p>
                <p style={{ fontSize: 12, color: "#9a8f7e", margin: "2px 0 0" }}>Preencha os dados abaixo</p>
              </div>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              ...
              <div>
                <label style={labelStyle}>Valor <span style={{ color: "#8B2246" }}>*</span></label>
                <div style={{ position: "relative" }}>
                  <DollarSign size={15} color="#9a8f7e" style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", pointerEvents: "none" }} />
                  <input type="number" min="0.01" step="0.01" placeholder="0,00" value={value}
              ...
              <div>
                <label style={labelStyle}>Descrição <span style={{ color: "#8B2246" }}>*</span></label>
                <div style={{ position: "relative" }}>
                  <FileText size={15} color="#9a8f7e" style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", pointerEvents: "none" }} />
                  <input type="text" placeholder="Ex: Salário, Freelance, Investimentos…" value={description} maxLength={255}
              ...
              <div>
                <label style={labelStyle}>Data <span style={{ color: "#8B2246" }}>*</span></label>
                <div style={{ position: "relative" }}>
                  <Calendar size={15} color="#9a8f7e" style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", pointerEvents: "none" }} />
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#F5F0E4", borderRadius: 12, padding: "14px 18px" }}>
                <div>
                  <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: "#1a1a1a" }}>Renda recorrente mensal</p>
                  <p style={{ margin: "2px 0 0", fontSize: 12, color: "#9a8f7e" }}>Marque se essa renda se repete todo mês</p>
                </div>
              ...
              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end", marginTop: 4 }}>
                <button onClick={() => navigate("/dashboard")} disabled={loading}
                  style={{ display: "flex", alignItems: "center", gap: 8, background: "#F5F0E4", border: "1.5px solid #D9D0BE", borderRadius: 100, padding: "11px 22px", fontSize: 13, fontWeight: 700, color: "#1a1a1a", cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: loading ? 0.6 : 1 }}>
                  Cancelar
                </button>
                <button onClick={handleSubmit} disabled={loading}
                  style={{ display: "flex", alignItems: "center", gap: 8, background: loading ? "#D9D0BE" : "linear-gradient(135deg, #C89B30 0%, #E8BE45 100%)", border: "none", borderRadius: 100, padding: "11px 28px", fontSize: 13, fontWeight: 700, color: "#1a1a1a", cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit" }}
                  onMouseEnter={(e) => { if (!loading) e.currentTarget.style.transform = "scale(1.02)"; }}
                  onMouseLeave={(e) => { e.currentTarget.style.transform = "scale(1)"; }}>
                  {loading ? (<><RefreshCw size={14} style={{ animation: "spin 1s linear infinite" }} /> Registrando…</>) : (<><CheckCircle size={14} /> Registrar Renda</>)}
                </button>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <div style={{ background: "#fff", borderRadius: 18, boxShadow: "0 2px 24px rgba(0,0,0,0.07)", padding: "24px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, fontWeight: 700, color: "#1a1a1a", margin: 0 }}>Últimas Rendas</h3>
                <button onClick={() => navigate("/dashboard")} style={{ display: "flex", alignItems: "center", gap: 4, background: "none", border: "none", fontSize: 12, color: "#C89B30", cursor: "pointer", fontWeight: 600 }}>
              ...
              {loadingHistory ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {recentIncomes.map((income) => (
                    <div key={income.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: "#FDFAF5", borderRadius: 12, border: "1px solid #F0EAD8" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg, #C89B30 0%, #E8BE45 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <TrendingDown size={16} color="#1a1a1a" />
                        </div>
                        <div>
                          <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#1a1a1a" }}>{income.description}</p>
                          <p style={{ margin: "2px 0 0", fontSize: 11, color: "#9a8f7e" }}>
                            {formatDate(income.date)}
                            {income.is_recurrent && <span style={{ marginLeft: 6, color: "#C89B30", fontWeight: 600 }}>• Recorrente</span>}
                          </p>
                        </div>
                      </div>
                      <span style={{ fontSize: 13, fontWeight: 700, color: "#2E7D32" }}>{formatCurrency(income.value)}</span>
                    </div>
                  ))}
                </div>
              ) : (
                <div style={{ textAlign: "center", padding: "20px 0" }}>
                  <p style={{ fontSize: 13, color: "#9a8f7e", margin: 0 }}>Nenhuma renda registrada ainda.</p>
                  <p style={{ fontSize: 11, color: "#9a8f7e", margin: "4px 0 0" }}>Registre sua primeira renda acima!</p>
                </div>
              )}
            </div>

            <div style={{ background: "linear-gradient(135deg, #1A1A1A 0%, #2a2a2a 100%)", borderRadius: 18, padding: "24px" }}>
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, fontWeight: 700, color: "#F5F0E4", margin: "0 0 16px" }}>💡 Dicas do Dia</h3>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {tips.map((tip, index) => (
                  <div key={index} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                    ...
                    <p style={{ margin: 0, fontSize: 12, color: "#B8B0A0", lineHeight: 1.5 }}>{tip.text}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: "#fff", borderRadius: 18, boxShadow: "0 2px 24px rgba(0,0,0,0.07)", padding: "24px" }}>
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, fontWeight: 700, color: "#1a1a1a", margin: "0 0 16px" }}>📊 Resumo do Mês</h3>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 700, color: "#2E7D32" }}>{formatCurrency(recentIncomes.reduce((acc, inc) => acc + inc.value, 0))}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 13, color: "#9a8f7e" }}>Recorrentes</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: "#C89B30" }}>{recentIncomes.filter(i => i.is_recurrent).length}</span>
                </div>
              </div>
            </div>
          </div>
